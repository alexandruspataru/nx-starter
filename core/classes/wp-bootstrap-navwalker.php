...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$cssClass	 = ($depth == 0) ? 'dropdown-menu' : 'dropdown-submenu';
			$output		.= "\n$indent<ul role=\"menu\" class=\" $cssClass submenu-level-$depth\">\n";
		}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '>';
				echo '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html_x( 'Add a menu', 'Front-End - Missing menu', 'nexus' ) . '</a></li>';
				echo '</ul>';
				if ( $container ) {
					echo '</' . esc_attr( $container ) . '>'; }
			}
